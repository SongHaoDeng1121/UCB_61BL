...
public class SLList {
    ...
    public void reverse() {
        ...
        IntListNode Head = helper(sentinel.next);
        sentinel.next = Head;
        // base cases: find the way to stop the recursion
    }

    public IntListNode helper(IntListNode node){
        //IntListNode temp = node.next.next;
        if (node.next == sentinel){
            return node;
        }
        IntListNode temp = helper(node.next);
        node.next.next = node;
        node.next = sentinel;
        return temp;
